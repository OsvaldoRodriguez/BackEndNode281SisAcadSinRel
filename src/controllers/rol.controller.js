...
export default {
  async mostrar(req, res) {
    try {
      const data = await models.Rol.findAll();
      res.status(200).json({ mensaje: "Todo OK", body: data });
    } catch (error) {
      console.log(error);
  ...
  async guardar(req, res) {
    try {
      ...
      const data = await models.Rol.create(req.body);
  ...
  async actualizar(req, res) {
    ...
    try {
      const data = await models.Rol.update(req.body, {
        where: {
          id: ID,
        },
      });
  ...
  async eliminar(req, res) {
    ...
    try {
      const data = await models.Rol.destroy({
        where: {
          id: ID,
        },
      });
