...
export default {
  ...
  async mostrar(req, res) {
    try {
      const data = await models.Usuario_Comenta_Actividad_Evento.findAll({
        // include : {
  ...
  async  guardar(req, res) {
    try {
      ...
      const data = await models.Usuario_Comenta_Actividad_Evento.create(req.body);
      res.status(200).json({ mensaje: "Todo Okey", body: data });
    } catch (error) {
      console.log(error);
  ...
  async actualizar(req, res) {
    ...
    try {
      const data = await models.Usuario_Comenta_Actividad_Evento.update(req.body, {
        where: {
          id: ID,
        },
      });
  ...
  async  eliminar(req, res) {
    ...
    try {
      const data = await models.Usuario_Comenta_Actividad_Evento.destroy({
        where: {
          id: ID,
        },
      });
