...
export default {
  ...
  async mostrar(req, res) {
    try {
      const data = await models.Foto.findAll({
        // include : {
  ...
  async  guardar(req, res) {
    try {
      ...
      const data = await models.Foto.create(req.body);
      res.status(200).json({ mensaje: "Todo Okey", body: data });
    } catch (error) {
      console.log(error);
  ...
  async actualizar(req, res) {
    ...
    try {
      const data = await models.Foto.update(req.body, {
        where: {
          id: ID,
        },
      });
  ...
  async  eliminar(req, res) {
    ...
    try {
      const data = await models.Foto.destroy({
        where: {
          id: ID,
        },
      });
