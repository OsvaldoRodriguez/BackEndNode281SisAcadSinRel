...
var _default = {
  async mostrar(req, res) {
    try {
      const data = await _models.default.Actividad_Evento.findAll({
        include: {
  ...
  async guardar(req, res) {
    try {
      ...
      const data = await _models.default.Actividad_Evento.create(req.body);

      // recuperando
      await data.reload();
      // console.log("datos recuperados", data.dataValues);
      await _models.default.Actividad_Evento_Has_Expositor.create({
    ...
    try {
      const data = await _models.default.Actividad_Evento.findAll({
        include: [{
          ...
          model: _models.default.Ambiente
        }, {
          model: _models.default.Expositor
        }],
        where: {
          EventoId: ID
        }
  ...
  async actualizar(req, res) {
    ...
    try {
      const data = await _models.default.Actividad_Evento.update(nuevos_datos, {
        where: {
          id: ID
        }
      });
      ...
      // console.log("datos recuperados", data.dataValues);
      await _models.default.Actividad_Evento_Has_Expositor.update({
        ExpositorId: req.body.ExpositorId
      }, {
        where: {
  ...
  async eliminar(req, res) {
    ...
    try {
      const data = await _models.default.Actividad_Evento.destroy({
        where: {
          id: ID
        }
      });
