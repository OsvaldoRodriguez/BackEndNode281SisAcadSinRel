...
var _default = {
  async mostrar(req, res) {
    try {
      const data = await _models.default.Institucion.findAll({
        // include : {
        //   // model : models.Evento
        // }
      });
  ...
  async guardar(req, res) {
    try {
      ...
      const data = await _models.default.Institucion.create(req.body);
      res.status(200).json({
        mensaje: "Todo Okey",
        body: data
      });
  ...
  async actualizar(req, res) {
    ...
    try {
      const data = await _models.default.Institucion.update(req.body, {
        where: {
          id: ID
        }
      });
  ...
  async eliminar(req, res) {
    ...
    try {
      const data = await _models.default.Institucion.destroy({
        where: {
          id: ID
        }
      });
