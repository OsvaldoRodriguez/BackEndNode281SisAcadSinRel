...
var _default = {
  async mostrar(req, res) {
    ...
    try {
      const data = await _models.default.Persona.findAll({
        // include : {
  ...
  async guardar(req, res) {
    try {
      ...
      const data = await _models.default.Persona.create(req.body);
      res.status(200).json({
        mensaje: "Todo Okey",
        body: data
      });
  ...
  async actualizar(req, res) {
    ...
    try {
      const data = await _models.default.Persona.update(req.body, {
        where: {
          id: ID
        }
      });
  ...
  async eliminar(req, res) {
    ...
    try {
      const data = await _models.default.Persona.destroy({
        where: {
          id: ID
        }
      });
