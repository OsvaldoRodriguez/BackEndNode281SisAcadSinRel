...
var _default = {
  async mostrar(req, res) {
    try {
      const data = await _models.default.Certificado.findAll({
        // include : {
  ...
  async guardar(req, res) {
    try {
      ...
      const data = await _models.default.Certificado.create(req.body);
      res.status(200).json({
        mensaje: "Todo Okey",
        body: data
      });
  ...
  async actualizar(req, res) {
    ...
    try {
      const data = await _models.default.Certificado.update(req.body, {
        where: {
          id: ID
        }
      });
  ...
  async eliminar(req, res) {
    ...
    try {
      const data = await _models.default.Certificado.destroy({
        where: {
          id: ID
        }
      });
