...
var _default = {
  async mostrar(req, res) {
    try {
      const data = await _models.default.Rol.findAll();
  ...
  async guardar(req, res) {
    try {
      ...
      const data = await _models.default.Rol.create(req.body);
      res.status(200).json({
        mensaje: "Todo Okey",
        body: data
      });
  ...
  async actualizar(req, res) {
    ...
    try {
      const data = await _models.default.Rol.update(req.body, {
        where: {
          id: ID
        }
      });
  ...
  async eliminar(req, res) {
    ...
    try {
      const data = await _models.default.Rol.destroy({
        where: {
          id: ID
        }
      });
