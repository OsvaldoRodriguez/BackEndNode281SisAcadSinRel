...
var _default = {
  async mostrar(req, res) {
    try {
      const data = await _models.default.Actividad_Evento_Has_Expositor.findAll({
        // include : {
  ...
  async guardar(req, res) {
    try {
      ...
      const data = await _models.default.Actividad_Evento_Has_Expositor.create(req.body);
      res.status(200).json({
        mensaje: "Todo Okey",
        body: data
      });
  ...
  async actualizar(req, res) {
    ...
    try {
      const data = await _models.default.Actividad_Evento_Has_Expositor.update(req.body, {
        where: {
          id: ID
        }
      });
  ...
  async eliminar(req, res) {
    ...
    try {
      const data = await _models.default.Actividad_Evento_Has_Expositor.destroy({
        where: {
          id: ID
        }
      });
