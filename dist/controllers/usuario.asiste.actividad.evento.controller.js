...
var _default = {
  async mostrar(req, res) {
    try {
      const data = await _models.default.Usuario_Asiste_Actividad_Evento.findAll({
        // include : {
  ...
  async guardar(req, res) {
    try {
      ...
      const data = await _models.default.Usuario_Asiste_Actividad_Evento.create(req.body);
      res.status(200).json({
        mensaje: "Todo Okey",
        body: data
      });
  ...
  async actualizar(req, res) {
    ...
    try {
      const data = await _models.default.Usuario_Asiste_Actividad_Evento.update(req.body, {
        where: {
          id: ID
        }
      });
  ...
  async eliminar(req, res) {
    ...
    try {
      const data = await _models.default.Usuario_Asiste_Actividad_Evento.destroy({
        where: {
          id: ID
        }
      });
