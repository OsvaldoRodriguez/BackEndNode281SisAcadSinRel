...
var _default = {
  async mostrar(req, res) {
    try {
      const data = await _models.default.Foto.findAll({
        // include : {
  ...
  async guardar(req, res) {
    try {
      ...
      const data = await _models.default.Foto.create(req.body);
      res.status(200).json({
        mensaje: "Todo Okey",
        body: data
      });
  ...
  async actualizar(req, res) {
    ...
    try {
      const data = await _models.default.Foto.update(req.body, {
        where: {
          id: ID
        }
      });
  ...
  async eliminar(req, res) {
    ...
    try {
      const data = await _models.default.Foto.destroy({
        where: {
          id: ID
        }
      });
