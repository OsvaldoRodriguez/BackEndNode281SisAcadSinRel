...
var _default = {
  async mostrar(req, res) {
    ...
    try {
      const data = await _models.default.Expositor.findAll({
        include: {
  ...
  async guardar(req, res) {
    try {
      ...
      const data = await _models.default.Expositor.create(req.body);
      res.status(200).json(data);
    } catch (error) {
      console.log(error);
      res.status(500).json({
  ...
  async actualizar(req, res) {
    ...
    try {
      const data = await _models.default.Expositor.update(req.body, {
        where: {
          id: ID
        }
      });
  ...
  async eliminar(req, res) {
    ...
    try {
      const data = await _models.default.Expositor.destroy({
        where: {
          id: ID
        }
      });
